...
namespace SevenWonder.CardGenerator
{
    ...
    {
        public static IList<CardMapping> GetMappingList()
        {
            var mappings = new List<CardMapping>();
            CreateAgeICards(mappings);
            CreateAgeIICards(mappings);
            CreateAgeIIICards(mappings);
            return mappings;
        }

        public static IList<IStructureCard> ReadAndMapXmlFile(string path)
        {
            CardCollection collection = null;
            using (var stream = new StreamReader(path))
            {
                var streamResult = stream.ReadToEnd();
                var xmlDoc = XDocument.Parse(streamResult);
                collection = Serializer.Deserialize<CardCollection>(xmlDoc.ToString());
            }
            IList<IStructureCard> result = new List<IStructureCard>();
            if (collection == null)
                return result;
            foreach (var item in collection.CardMapping)
                result.Add(StructureCardFactory.CreateStructureCard(item));
            return result;
        }
